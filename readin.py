...
def crop(image):
    ...
    imf = autoCrop(background)
    return imf


if __name__ == '__main__':

    im1 = Image.open('data/1.png')
    im1 = crop(im1)

    im2 = Image.open('data/1-1.png')
    im2 = crop(im2)

    print('image sizes: ' + str(im1.size) + str(im2.size))

    diff = ImageChops.difference(im1, im2)
    im1.show()
    im2.show()

    diff = diff.filter(ImageFilter.CONTOUR)
    diff.show()
